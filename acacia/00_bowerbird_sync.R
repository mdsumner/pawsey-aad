...
src0 <- sources("Artist AMSR2 near-real-time 3.125km sea ice concentration") %>%
      bb_modify_source(      source_url = paste0("https://seaice.uni-bremen.de/data/amsr2/asi_daygrid_swath/s3125/", format(Sys.Date(), "%Y"), "/"),
      method = list(reject_follow = rf, accept_download = ".*tif$", no_host = FALSE, target_s3_args = list(bucket = "idea-amsr2-asi-s3125")))
...
#src0 <- blueant::sources("CMEMS global gridded SSH near-real-time") %>% bb_modify_source(user = secret00$user, password = secret00$password,
#                                                                       method = list(accept_follow = year00, accept_download = ".*nc$", no_host = FALSE,
